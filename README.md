...
| `complete_job`    | Archives the job, deletes it from `pgwf.jobs`, removes the job_id from dependents, and wakes listeners.                | `complete_job(tenant_id TEXT, job_id TEXT, lease_id TEXT, worker_id TEXT, completion_status TEXT DEFAULT 'succeeded', failure_detail TEXT DEFAULT NULL)` |
| `complete_unheld_job` | Archives a `READY` job that locking and completing in a single op (and unblocking dependent work/notifying as needed). | `complete_unheld_job(tenant_id TEXT, job_id TEXT, worker_id TEXT, completion_status TEXT DEFAULT 'succeeded', failure_detail TEXT DEFAULT NULL)`         |
...
7. When the job is complete, the GPU worker calls complete_job() to mark the job as complete (optionally reporting success/failure).
...
6. **Complete** – After writing the final payload back to the journal, the worker calls `pgwf.complete_job(tenant_id, job_id, lease_id, worker_id, completion_status, failure_detail)`. pgwf archives the row, deletes the live copy, removes the job_id from any dependent `wait_for` arrays, and emits NOTIFY signals to wake listeners (if enabled).
   Allowed `completion_status` values are `succeeded` and `failed`. `failure_detail` is only accepted when `completion_status = 'failed'` and is stored in `jobs_archive`.
